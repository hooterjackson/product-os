...
def build(root, out_dir):
    ...
    # The agent-facing surface. Emitted here too so `build/` is a complete
    # preview of what gets deployed, and generated by the same function that
    # writes the committed copy -- one generator, never two.
    publish_mod.generate(model.root, model, out_dir, volatile=True)
    return model
