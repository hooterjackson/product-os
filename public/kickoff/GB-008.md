...
# GB-008 — Bring the fixture up on the radio and prove it still behaves like a light (Z-M3)
...
- claude-code · work-laptop · Fix stale product-os entry in state/repos.js
  ...
  A verified way back exists on work-laptop, but it names that machine's paths and the session id, so it is not republished. In your product-os clone THERE, run `python3 tools/kickoff.py GB-008`.
