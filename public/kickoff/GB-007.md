...
# GB-007 — Build the rollback ladder, so a bad radio update can be backed out (Z-M2)
...
- claude-code · work-laptop · Fix stale product-os entry in state/repos.js
  ...
  A verified way back exists on work-laptop, but it names that machine's paths and the session id, so it is not republished. In your product-os clone THERE, run `python3 tools/kickoff.py GB-007`.
