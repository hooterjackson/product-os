...
- claude-code · work-laptop · Fix stale product-os entry in state/repos.js
  ...
  A verified way back exists on work-laptop, but it names that machine's paths and the session id, so it is not republished. Run `python3 tools/kickoff.py GB-011` there.
