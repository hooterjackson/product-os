...
You have been working on **`GB-014` — Un-park Zigbee — the decision D15 leaves open**.
...
1. **Two lines, in two files. This split is deliberate:**
   ...
   ```
   GB-014: <this machine> <today, YYYY-MM-DD>
   ```

   `state/threads/manual.local.yaml` is git-ignored. The URL goes there:

   ```
   GB-014: <the URL of THIS chat, from your browser's address bar>
   ```
...
2. **Write what this chat actually achieved** into `GB-014`'s
