...
You have been working on **`GB-012` — Gate L — the fixture joins and holds as a light**.
...
1. **Two lines, in two files. This split is deliberate:**
   ...
   ```
   GB-012: <this machine> <today, YYYY-MM-DD>
   ```

   `state/threads/manual.local.yaml` is git-ignored. The URL goes there:

   ```
   GB-012: <the URL of THIS chat, from your browser's address bar>
   ```
...
2. **Write what this chat actually achieved** into `GB-012`'s
