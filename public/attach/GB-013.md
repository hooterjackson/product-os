...
You have been working on **`GB-013` — Stand up the HA-side Zigbee OTA server**.
...
1. **Two lines, in two files. This split is deliberate:**
   ...
   ```
   GB-013: <this machine> <today, YYYY-MM-DD>
   ```

   `state/threads/manual.local.yaml` is git-ignored. The URL goes there:

   ```
   GB-013: <the URL of THIS chat, from your browser's address bar>
   ```
...
2. **Write what this chat actually achieved** into `GB-013`'s
