...
You have been working on **`Q-004` — Where do 300 liveness taps a second come from?**.
...
1. **Two lines, in two files. This split is deliberate:**
   ...
   ```
   Q-004: <this machine> <today, YYYY-MM-DD>
   ```

   `state/threads/manual.local.yaml` is git-ignored. The URL goes there:

   ```
   Q-004: <the URL of THIS chat, from your browser's address bar>
   ```
...
2. **Write what this chat actually achieved** into `Q-004`'s
