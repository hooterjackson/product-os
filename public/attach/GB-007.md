...
You have been working on **`GB-007` — Build the rollback ladder, so a bad radio update can be backed out (Z-M2)**.
...
1. **Two lines, in two files. This split is deliberate:**
   ...
   ```
   GB-007: <this machine> <today, YYYY-MM-DD>
   ```

   `state/threads/manual.local.yaml` is git-ignored. The URL goes there:

   ```
   GB-007: <the URL of THIS chat, from your browser's address bar>
   ```
...
2. **Write what this chat actually achieved** into `GB-007`'s
