...
You have been working on **`GB-005` — Prepare the radio build: memory layout, and a stub for signing updates (Z-M1)**.
...
1. **Two lines, in two files. This split is deliberate:**
   ...
   ```
   GB-005: <this machine> <today, YYYY-MM-DD>
   ```

   `state/threads/manual.local.yaml` is git-ignored. The URL goes there:

   ```
   GB-005: <the URL of THIS chat, from your browser's address bar>
   ```
...
2. **Write what this chat actually achieved** into `GB-005`'s
