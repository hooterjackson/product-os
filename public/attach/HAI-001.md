...
You have been working on **`HAI-001` — Rotate the GPU box's Linux password**.
...
1. **Two lines, in two files. This split is deliberate:**
   ...
   ```
   HAI-001: <this machine> <today, YYYY-MM-DD>
   ```

   `state/threads/manual.local.yaml` is git-ignored. The URL goes there:

   ```
   HAI-001: <the URL of THIS chat, from your browser's address bar>
   ```
...
2. **Write what this chat actually achieved** into `HAI-001`'s
