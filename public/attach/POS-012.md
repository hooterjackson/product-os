...
You have been working on **`POS-012` — A correction is only real where it is read**.
...
1. **Two lines, in two files. This split is deliberate:**
   ...
   ```
   POS-012: <this machine> <today, YYYY-MM-DD>
   ```

   `state/threads/manual.local.yaml` is git-ignored. The URL goes there:

   ```
   POS-012: <the URL of THIS chat, from your browser's address bar>
   ```
...
2. **Write what this chat actually achieved** into `POS-012`'s
