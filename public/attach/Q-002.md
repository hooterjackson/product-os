...
You have been working on **`Q-002` — Does a mid-move retarget re-plan smoothly, or stop and restart?**.
...
1. **Two lines, in two files. This split is deliberate:**
   ...
   ```
   Q-002: <this machine> <today, YYYY-MM-DD>
   ```

   `state/threads/manual.local.yaml` is git-ignored. The URL goes there:

   ```
   Q-002: <the URL of THIS chat, from your browser's address bar>
   ```
...
2. **Write what this chat actually achieved** into `Q-002`'s
