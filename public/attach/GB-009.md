...
You have been working on **`GB-009` — Decide how the fixture is commissioned, and what a reset actually resets (Z-M4)**.
...
1. **Two lines, in two files. This split is deliberate:**
   ...
   ```
   GB-009: <this machine> <today, YYYY-MM-DD>
   ```

   `state/threads/manual.local.yaml` is git-ignored. The URL goes there:

   ```
   GB-009: <the URL of THIS chat, from your browser's address bar>
   ```
...
2. **Write what this chat actually achieved** into `GB-009`'s
