...
You have been working on **`GB-010` — Prove the deployed fixture only moves when it is meant to (Z-M5)**.
...
1. **Two lines, in two files. This split is deliberate:**
   ...
   ```
   GB-010: <this machine> <today, YYYY-MM-DD>
   ```

   `state/threads/manual.local.yaml` is git-ignored. The URL goes there:

   ```
   GB-010: <the URL of THIS chat, from your browser's address bar>
   ```
...
2. **Write what this chat actually achieved** into `GB-010`'s
