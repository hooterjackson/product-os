...
You have been working on **`GB-008` — Bring the fixture up on the radio and prove it still behaves like a light (Z-M3)**.
...
1. **Two lines, in two files. This split is deliberate:**
   ...
   ```
   GB-008: <this machine> <today, YYYY-MM-DD>
   ```

   `state/threads/manual.local.yaml` is git-ignored. The URL goes there:

   ```
   GB-008: <the URL of THIS chat, from your browser's address bar>
   ```
...
2. **Write what this chat actually achieved** into `GB-008`'s
