...
You have been working on **`Q-001` — Does Spot Mode (Auto/Hold/Manual) survive gate A?**.
...
1. **Two lines, in two files. This split is deliberate:**
   ...
   ```
   Q-001: <this machine> <today, YYYY-MM-DD>
   ```

   `state/threads/manual.local.yaml` is git-ignored. The URL goes there:

   ```
   Q-001: <the URL of THIS chat, from your browser's address bar>
   ```
...
2. **Write what this chat actually achieved** into `Q-001`'s
