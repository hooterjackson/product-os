...
You have been working on **`GB-001` — Finish the M5 fault ring: a real fault storm, and the brownout case**.
...
1. **Two lines, in two files. This split is deliberate:**
   ...
   ```
   GB-001: <this machine> <today, YYYY-MM-DD>
   ```

   `state/threads/manual.local.yaml` is git-ignored. The URL goes there:

   ```
   GB-001: <the URL of THIS chat, from your browser's address bar>
   ```
...
2. **Write what this chat actually achieved** into `GB-001`'s
