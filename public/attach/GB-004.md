...
You have been working on **`GB-004` — M6 on hardware: the motor-silent limb, MUTE-CLEAR, and the armed lane**.
...
1. **Two lines, in two files. This split is deliberate:**
   ...
   ```
   GB-004: <this machine> <today, YYYY-MM-DD>
   ```

   `state/threads/manual.local.yaml` is git-ignored. The URL goes there:

   ```
   GB-004: <the URL of THIS chat, from your browser's address bar>
   ```
...
2. **Write what this chat actually achieved** into `GB-004`'s
