...
You have been working on **`GB-006` — Add the single-slot mailbox the radio and the safety loop pass messages through (Z-M0)**.
...
1. **Two lines, in two files. This split is deliberate:**
   ...
   ```
   GB-006: <this machine> <today, YYYY-MM-DD>
   ```

   `state/threads/manual.local.yaml` is git-ignored. The URL goes there:

   ```
   GB-006: <the URL of THIS chat, from your browser's address bar>
   ```
...
2. **Write what this chat actually achieved** into `GB-006`'s
