...
You have been working on **`GB-011` — Report telemetry over the radio, including the long dumps (Z-M6)**.
...
1. **Two lines, in two files. This split is deliberate:**
   ...
   ```
   GB-011: <this machine> <today, YYYY-MM-DD>
   ```

   `state/threads/manual.local.yaml` is git-ignored. The URL goes there:

   ```
   GB-011: <the URL of THIS chat, from your browser's address bar>
   ```
...
2. **Write what this chat actually achieved** into `GB-011`'s
